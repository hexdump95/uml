$.ajax({
        ...
        success: (res) => {
            ...
            jsonRes.connectors.forEach(connector => {
                const link = LinkFactory.createLink(connector.type);
                link.source(
                    graph.getElements().find(e => e.id === connector.source.id),
                    {port: connector.source.port},
                );
                link.target(
                    graph.getElements().find(e => e.id === connector.target.id),
